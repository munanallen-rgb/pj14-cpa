...
Use focused package tests for narrow changes. Do not create or delete runtime/auth data as part of verification.

## Architecture Map

- `cmd/server/`: process entrypoint.
- `internal/api/`: HTTP API, routes, middleware, and management handlers.
- `internal/runtime/executor/`: provider execution.
- `internal/translator/`: protocol translation.
- `internal/thinking/`: reasoning configuration pipeline.
- `internal/auth/`, `internal/config/`, `internal/registry/`, `internal/store/`, `internal/watcher/`: owned support modules.
- `sdk/`: embeddable public API.
- `test/`: cross-module compatibility tests.
